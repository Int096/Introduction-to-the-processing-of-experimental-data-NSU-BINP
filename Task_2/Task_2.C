...
void readDataFromFileToHist2D(std::string filename, TH2D *hist)
{
    ...
    {
        std::cerr << "Возникли проблемы при попытке прочтения файла " 
                  << filename << "." << std::endl;
        exit(-1);
    }
}
...
Int_t Task_2()
{
    ...
    TH2D *experiment_hist = new TH2D("experiment_hist",
                                     "Experiment (style = surf3)",
                                     nbinX, x_min, x_max,
                                     nbinY, y_min, y_max);
    
    TH2D *simulation_hist = new TH2D("simulation_hist",
                                     "Simulation (sytle = colz)",
                                     nbinX, x_min, x_max,
                                     nbinY, y_min, y_max);


    readDataFromFileToHist2D("./data/fMC.dat",   simulation_hist);
    readDataFromFileToHist2D("./data/fdata.dat", experiment_hist);
    ...
    //------------------------------------------------------------------------------------------//
    // Гистограмма результатов эксперимента 
    //------------------------------------------------------------------------------------------//
    canvas->cd(1);
    ...
    //------------------------------------------------------------------------------------------//
    // Вывод резальтатов эксперимента
    //------------------------------------------------------------------------------------------//
    canvas->cd(2);
